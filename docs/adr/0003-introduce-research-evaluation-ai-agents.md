...
AI Agentはminimal run envelopeとread-only Research Tool Serverで、市場状態、指標、Candidate Strategy成績、reject履歴、memoryを参照する。出力は `AgentObservation`、`StrategyProposal`、`CandidateReview`、`AgentMemoryWrite` に分ける。host側のAgentOutputProcessorがschema validation、Strategy DSL validation、Risk Gate validation、memory保存、proposal永続化を行い、StrategyEvaluationPipelineがCandidate Strategy用Paper Account作成、Paper Trading評価、Baseline Strategy昇格 / Candidate Strategy停止判断を行う。
